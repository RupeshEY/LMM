...
class ClaimRetrieveRsCommentCreatorTest extends GUnitTestClass {

  private var _entityMapperService = new EntityMapperService()

  function testNewClaimRetrieveRsWithEmptyComments() {
    TestDataUtil.runWithExistingBundleNoCommit(TestDataUtil.createNonPersistentBundle(), \-> {
      TestDataUtil.runAsUser(User.util.UnrestrictedUser, \-> {
        ...
        // When
        final var ecfMessage = _entityMapperService.createEntity(xmlPayload) as ECFMessageClaimRetrieveRs_Ext
  ...
  function testNewClaimRetrieveRsWithoutComments() {
    TestDataUtil.runWithExistingBundleNoCommit(TestDataUtil.createNonPersistentBundle(), \-> {
      TestDataUtil.runAsUser(User.util.UnrestrictedUser, \-> {
        // Given
        final var xmlPayload = ECFMessageUtil.createNewClaimRetrieveRs()
  ...
  function testNewClaimRetrieveRsWithComments() {
    TestDataUtil.runWithExistingBundleNoCommit(TestDataUtil.createNonPersistentBundle(), \-> {
      TestDataUtil.runAsUser(User.util.UnrestrictedUser, \-> {
        final var xmlPayload = ECFMessageUtil.createNewClaimRetrieveRs()
        final var ecfMessage = _entityMapperService.createEntity(xmlPayload) as ECFMessageClaimRetrieveRs_Ext
