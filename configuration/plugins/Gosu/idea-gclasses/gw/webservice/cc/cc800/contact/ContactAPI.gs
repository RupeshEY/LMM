...
class ContactAPI implements ABClientAPI {
  ...
  override function pendingUpdateApproved(context : ABClientAPIPendingChangeContext, abuidContainer : ABClientAPIAddressBookUIDContainer) {
    ...
    var contact = Query.make(Contact).compare("PublicId", Equals, context.PublicID).select().AtMostOneRow
    if(contact != null) {
      try {
  ...
  private function createRejectionActivity(context: ABClientAPIPendingChangeContext, pattern: ActivityPattern ){
    ...
    //find user
    var selectedUser = Query.make(User).join("Credential").compare("UserName", Equals, context.Username).select().first()
    
    var selectedClaim : Claim
    if (context.RootEntityID.HasContent) {
      selectedClaim = Query.make(Claim).compare("PublicID", Equals, context.RootEntityID).select().AtMostOneRow
    }
    
    //find claim contact
    var claimContact = Query.make(ClaimContact).join("Contact").compare("PublicID", Equals, context.PublicID).select().first()

    if (claimContact == null) {
      var contactQuery = Query.make(ClaimContact)
      var claimTableQuery = contactQuery.join("Claim").compare("PublicID", Equals, context.RootEntityID)
      var contactTableQuery = contactQuery.join("Contact").compare("AddressBookUID", Equals, context.AddressBookUID)
