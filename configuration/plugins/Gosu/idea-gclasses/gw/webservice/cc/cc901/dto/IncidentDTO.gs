 ...
 * A(n) {@link gw.webservice.cc.cc901.dto.IncidentDTO} is a DTO that represents an instance of {@link entity.Incident} for use by certain WS-I Web Services in the base product. This file is composed of basic getters and setters, plus members that make it easier for server-side code to work with DTOs, like virtual getters that query for entities by PublicID, and methods to copy fields to or from an instance of {@link entity.Incident}.
 ...
 * <p>The specific mappings for {@link gw.webservice.cc.cc901.dto.IncidentDTO} are as follows:
 ...
 */
@Export
@WsiExportable("http://guidewire.com/cc/ws/gw/webservice/cc/cc901/dto/IncidentDTO")
final class IncidentDTO {
   ...
   * @param that The Incident to copy from.
   */
  final function readFrom(that : Incident) : gw.webservice.cc.cc901.dto.IncidentDTO {
  ...
  /** Convenience property that returns the {@link entity.Address} whose PublicID is {@code LocationAddressPublicID}, or {@code null} if PublicID is {@code null}. This property is only available on the server--it is not exposed through the WS-I layer. */
  property get LocationAddress()      : Address              { return fetchByPublicID(LocationAddressPublicID) }

  /** Convenience property that returns a {@link entity.TypeofProperty[]} of the objects whose PublicIDs are in {@code TypeofPropertyPublicIDs}. This property is only available on the server--it is not exposed through the WS-I layer. */
   ...
   */
  static function valueOf(that : Incident) : gw.webservice.cc.cc901.dto.IncidentDTO {
    return new gw.webservice.cc.cc901.dto.IncidentDTO().readFrom(that)
  }
