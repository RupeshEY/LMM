 ...
 */
  public static function populateContactFromSearchResult(contact:Contact, searchResult:ABContactAPISearchResultContainer_Results_Entry) {
    ...
    if (contact typeis Person) {
      ...
      contact.CellPhoneCountry = typekey.PhoneCountryCode.get(searchResult.CellPhoneCountry)
    ...
    contact.FaxPhoneCountry = typekey.PhoneCountryCode.get(searchResult.FaxPhoneCountry)
    contact.FaxPhoneExtension = searchResult.FaxPhoneExtension
    contact.HomePhone     = searchResult.HomePhone
    contact.HomePhoneCountry = typekey.PhoneCountryCode.get(searchResult.HomePhoneCountry)
    contact.HomePhoneExtension = searchResult.HomePhoneExtension
    contact.WorkPhone     = searchResult.WorkPhone
    contact.WorkPhoneCountry = typekey.PhoneCountryCode.get(searchResult.WorkPhoneCountry)
    ...
    contact.CreateStatus = typekey.ContactCreationApprovalStatus.get(searchResult.CreateStatus)
  }
 ...
 */
  public static function populateContactFromDuplicateResult(contact:Contact, duplicateResult:ABContactAPIFindDuplicatesResultContainer_Results_Entry) {
    ...
    if (contact typeis Person) {
      ...
      contact.Prefix      = typekey.NamePrefix.get(duplicateResult.Prefix)
      contact.Suffix      = typekey.NameSuffix.get(duplicateResult.Suffix)
      contact.Particle    = duplicateResult.Particle
      contact.CellPhone   = duplicateResult.CellPhone
      contact.CellPhoneCountry = typekey.PhoneCountryCode.get(duplicateResult.CellPhoneCountry)
    ...
    contact.PrimaryPhone  = typekey.PrimaryPhoneType.get(duplicateResult.PrimaryPhone)
    contact.FaxPhone      = duplicateResult.FaxPhone
    contact.FaxPhoneCountry = typekey.PhoneCountryCode.get(duplicateResult.FaxPhoneCountry)
    contact.FaxPhoneExtension = duplicateResult.FaxPhoneExtension
    contact.HomePhone     = duplicateResult.HomePhone
    contact.HomePhoneCountry = typekey.PhoneCountryCode.get(duplicateResult.HomePhoneCountry)
    contact.HomePhoneExtension = duplicateResult.HomePhoneExtension
    contact.WorkPhone     = duplicateResult.WorkPhone
    contact.WorkPhoneCountry = typekey.PhoneCountryCode.get(duplicateResult.WorkPhoneCountry)
    contact.WorkPhoneExtension = duplicateResult.WorkPhoneExtension
    contact.Preferred     = duplicateResult.Preferred
    contact.VendorType    = typekey.VendorType.get(duplicateResult.VendorType)
    ...
    address.State          = typekey.State.get(duplicateResult.PrimaryAddress.State)
    ...
    address.Country        = typekey.Country.get(duplicateResult.PrimaryAddress.Country)
    address.County         = duplicateResult.PrimaryAddress.County
    address.Description    = duplicateResult.PrimaryAddress.Description
    address.AddressType    = typekey.AddressType.get(duplicateResult.PrimaryAddress.AddressType)
    ...
    address.GeocodeStatus  = typekey.GeocodeStatus.get(duplicateResult.PrimaryAddress.GeocodeStatus)
    contact.PrimaryAddress = address
  }
}
