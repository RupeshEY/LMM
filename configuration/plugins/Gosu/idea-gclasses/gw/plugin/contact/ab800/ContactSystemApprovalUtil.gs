 ...
 * </ul>
 * @deprecated Since 9.0.0 use gw.plugin.contact.ContactSystemApprovalUtil instead
 */
@Export
class ContactSystemApprovalUtil {
  ...
  public static function shouldCreateInContactSystem(theContact : Contact) : boolean {
    ...
    if(cc != null) {
      ...
      */
      LOGGER.trace("Found claimcontact for ${theContact}, shouldCreate is " + shouldCreate)
    } else {
      // not a claim contact - don't create
      LOGGER.trace("Did not find claimcontact for ${theContact}, not creating in CM")
      shouldCreate = false
    }
    return shouldCreate
  }
