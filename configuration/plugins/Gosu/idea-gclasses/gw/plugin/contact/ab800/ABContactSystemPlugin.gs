...
uses gw.contactmapper.ab800.CCPropertyMapping
...
class ABContactSystemPlugin implements ContactSystemPlugin {
  ...
  private var _mappingsForTypes : LoadingCache<IEntityType, Collection<CCPropertyMapping>>  =
      CacheBuilder/*<IEntityType,Collection<CCPropertyMapping>>*/.newBuilder().build(
          new CacheLoader<IEntityType, Collection<CCPropertyMapping>>() {
            override function load(type : IEntityType) : Collection<CCPropertyMapping> {
              return computeAllMappingsForType(type);
            }
          }
      );
  ...
  /**
   * In ClaimCenter8 there is no support for contact documents
   * @return false, there is no support for contact documents in CC8
   */
  ...
  */
      ...
      override function visit(node: entity.KeyableBean) {
        if (node typeis AddressBookLinkable) {
          if (node.ExternalLinkID == null && node.getFieldValue("AddressBookUID") == null) {
            var newLinkID = getNextExternalID()
  ...
  private function getNextExternalID() : String {
    return CCConfigParameters.ExternalLinkIDPrefix.Value + ":" + gw.api.system.database.SequenceUtil.next(1, "CONTACT_EXTERNALLINKID") as String
  }
  ...
  private function fixXmlForNewLinkIDs(bean : KeyableBean, xml: XmlBackedInstance) : XmlBackedInstance {
    ...
    if(xml.LinkID == null) {
      if(beanLinkID != null) {
        ...
        bean.setFieldValue("ExternalLinkID", externalID)
        xml.updateFieldValue(MappingConstants.EXTERNAL_UNIQUE_ID, externalID)
      }
    }
    var typeMappings  : java.util.Collection<gw.contactmapper.ab800.CCPropertyMapping>

    try {
      typeMappings = _mappingsForTypes.get(bean.IntrinsicType)
    } catch (ee: ExecutionException){
      typeMappings = computeAllMappingsForType(bean.IntrinsicType)
    }

    for(fk in xml.Fk) {
      var fkMapping = typeMappings.firstWhere( \ elt -> elt.ABName == fk.Name)
      var propName = fkMapping == null ? fk.Name : fkMapping.Property.PropertyInfo.Name
      var fkBean = bean[propName] as KeyableBean
      if(fkBean != null) {
        fk.XmlBackedInstance = fixXmlForNewLinkIDs(fkBean, fk.XmlBackedInstance)
      }
    }
    for(array in xml.Array) {
      var arrayMapping = typeMappings.firstWhere( \ elt -> elt.ABName == array.Name)
      var propName = arrayMapping == null ? array.Name : arrayMapping.Property.PropertyInfo.Name
      var beanArray = bean[propName] as KeyableBean[]
  ...
  private function computeAllMappingsForType(type: IEntityType): Collection <CCPropertyMapping> {
    return handleErrors(null, null, \ -> {
      var mappingsForType = new HashSet<CCPropertyMapping>()

      var allMappings = _mapper.Mappings
      for(mapping in allMappings) {
        if(gw.api.util.TypeUtil.isNominallyOrStructurallyAssignable(mapping.Property.RootType, type)) {
  ...
  private function mergeRelatedContactsIntoContact(contact : Contact, relContacts : RelatedContactInfoContainer) : Contact {
    ...
    for (relContactEntry in relContacts.SourceRelatedContacts.Entry) {
      var contactRel = _contactRelationships.getRelationshipType(typekey.ContactBidiRel.get(relContactEntry.Relationship))
