...
class RepositoryDownloadRequestIntegrationTest extends GUnitTestClass {

  private static final var LOGGER = ECFConstants.LOGGER_BLOCK()

  function testSoapEnvelopeCreated() {
    var documentDownlaodEntity = ECFMessageEntityUtil.createDocumentDownloadRq()
    var repositoryOperationRqMapper = new RepositoryOperationRqMapperFactory().getMapper(documentDownlaodEntity)
    ...
    var repositoryDownloadRequest = RepositoryOperationRq.parse(attachment.getRawContentBytes())
    assertNotNull(repositoryDownloadRequest)
