...
class IMRDocumentDownloadRqServiceTest extends GUnitTestClass {

  private static var _downloadRqService: IMRDocumentDownloadRqService

  function beforeMethod() {
    super.beforeMethod()
    _downloadRqService = new IMRDocumentDownloadRqService()
  }

  function testGeneratePayloadIntegration() {
    var entity = ECFMessageEntityUtil.createDocumentDownloadRq()
    var payload = _downloadRqService.generatePayload(entity)
  ...
  function testSignPayload() {
    var entity = ECFMessageEntityUtil.createDocumentDownloadRq()
    var payload = _downloadRqService.generatePayload(entity)
    payload = _downloadRqService.signPayload(payload)
  ...
  function testSendOutboundMessage() {
    var docDownloadMessage = ECFMessageEntityUtil.createDocumentDownloadRq()
    _downloadRqService._producerService = Mockito.mock(IMRProducerService)
    var soapMessage = _downloadRqService.generatePayload(docDownloadMessage)
    _downloadRqService.sendOutboundMessage(soapMessage)
    Mockito.verify(_downloadRqService._producerService).queueMessage(soapMessage)
  }
  ...
  function testSendOutboundMessage_Exception() {
    ...
    _downloadRqService._producerService = Mockito.mock(IMRProducerService)
    var soapMessage = _downloadRqService.generatePayload(docDownloadMessage)
    Mockito.doThrow(JMSException).when(_downloadRqService._producerService).queueMessage(soapMessage)
    assertCausesException(\-> _downloadRqService.sendOutboundMessage(soapMessage), IMRException)
  }
