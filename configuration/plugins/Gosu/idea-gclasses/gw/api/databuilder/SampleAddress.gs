...
class SampleAddress
{
  ...
  static property get Pasadena() : SampleAddress {
    return new SampleAddress("91101", "Pasadena", State.get("CA")).withGeocode(TC_POSTALCODE, new SpatialPoint(-118.1324, 34.1425).WellKnownText)}
  static property get Pasadena_LakeAve() : SampleAddress {
    return new SampleAddress("91101-3009", "Pasadena", State.get("CA"), "225 S. LAKE AVE, STE 300").withGeocode(TC_EXACT, new SpatialPoint(-118.1324, 34.1425).WellKnownText)}
  static property get SanDiego() : SampleAddress {
    return new SampleAddress(null, "San Diego", State.get("CA")).withGeocode(TC_CITY, new SpatialPoint(-117.0999, 32.7823).WellKnownText)}
  static property get SF_BealeSt() : SampleAddress {
    return new SampleAddress("94104", "San Francisco", State.get("CA"), "3220 Beale St").withGeocode(TC_EXACT, new SpatialPoint(-122.3972, 37.7925).WellKnownText)}
  static property get SanJose() : SampleAddress {
    return new SampleAddress("95126", "San Jose", State.get("CA"), null).withGeocode(TC_EXACT, new SpatialPoint(-121.8991, 37.3254).WellKnownText)  }
  static property get PheonixAZ_Perf() : SampleAddress {
    return new SampleAddress("85012", "PHOENIX",State.get("AZ"), "4000 N. CENTRAL AVE.").withGeocode(TC_EXACT, new SpatialPoint(-112.0738, 33.4939).WellKnownText)}
