...
@Export
class FinSumClaimantLeaf extends FinSumClaimantExposure implements Leaf {
  ...
  override function calculateAmounts(columns: List<String>) {
    columns.each(\column -> {
      ...
      var f = obtainUnrestrictedFinancialsCalculator(financialsExpression)
          .withExposure(_exposure.get()) //must not be null since excluded buckets for claim-level reserves already.
  ...
  override function buildSummaryInstance(): Node {
    return new FinSumClaimantExposure (_exposure, _claim)
  }

  override function compareTo(o: Node): int {
    var node = o as FinSumClaimantLeaf
  ...
  override function equals(obj: Object): boolean {
    ...
    if (!(obj typeis FinSumClaimantLeaf )) {
      return false
    }
    var rhs = obj as FinSumClaimantLeaf
