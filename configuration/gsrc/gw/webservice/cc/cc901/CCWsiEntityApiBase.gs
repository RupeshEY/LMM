package gw.webservice.cc.cc901
...
uses gw.webservice.cc.cc901.dto.ClaimContactDTO
uses gw.webservice.cc.cc901.dto.ClaimContactRoleDTO
...
uses gw.webservice.cc.cc901.dto.IncidentDTO
 ...
 */
@WsiWebService("http://guidewire.com/cc/ws/gw/webservice/cc/cc901/CCWsiEntityApiBase")
...
abstract class CCWsiEntityApiBase {
  ...
  protected function assembleFromDTO(claimDTO : ClaimDTO, policyDTO : PolicyDTO, bundle : Bundle) : Claim {
    var policy = policyDTO == null ? null : assembleFromDTO(policyDTO, bundle)
  ...
  protected function assembleFromDTO(claimDTO : ClaimDTO, policy : Policy, bundle : Bundle) : Claim {
    ...
    assembleFromDTOs(claimDTO.NewContacts,   claim)
    ...
    if (claimDTO.Newclaimant != null){
      assembleFromDTO(claimDTO.Newclaimant, claim, ContactRole.TC_CLAIMANT)
    }
    return claim
  }

  protected function assembleFromDTO(dto: PolicyDTO, bundle: Bundle): Policy {
    var policy = dto.writeToNewEntityIn(bundle)

    if (dto.Newinsured != null){
      assembleFromDTO(dto.Newinsured, policy, ContactRole.TC_INSURED)
    }
    return policy
  }

  protected function assembleFromDTO(dto: ContactDTO, policy: Policy, contactRole: ContactRole) {
    var contact = dto.writeToNewEntityIn(policy.Bundle)

    if (policy.isExclusiveRole(contactRole)) {
      policy.setContactByRole(contactRole, contact)
    } else {
      policy.addRole(contactRole, contact)
    }
  }
  ...
  protected function assembleFromDTO(dto: ContactDTO, claimContact: ClaimContact, bundle: Bundle): Contact {
    var contact = dto.writeToNewEntityIn(bundle)
    claimContact.setContact(contact)
    return contact
  }

  protected function assembleFromDTO(dto: ContactDTO, claim: Claim, contactRole: ContactRole) {
    var contact = dto.writeToNewEntityIn(claim.Bundle)

    if (claim.isExclusiveRole(contactRole)) {
      claim.setContactByRole(contactRole, contact)
    } else {
      claim.addRole(contactRole, contact)
    }
  }
  ...
  protected function assembleFromDTOs(dtos: ClaimContactRoleDTO[], claimContact: ClaimContact, claim:Claim,  bundle: Bundle): ClaimContactRole[] {
    return dtos.map(\dto -> {
      var claimContactRole = dto.writeToNewEntityIn(bundle)
      ...
      if (claim.isExclusiveRole(claimContactRole.Role)) {
        claim.setContactByRole(claimContactRole.Role, claimContact.Contact)
      } else {
        claim.addRole(claimContactRole.Role, claimContact.Contact)
      }
  ...
  protected function assembleFromDTOs(dtos: ClaimContactDTO[], claim: Claim): ClaimContact[] {
    return dtos.map(\dto -> {
      var claimContact = dto.writeToNewEntityIn(claim.Bundle)
      claimContact.Claim = claim

      if (dto.NewContact != null && dto.NewRoles.length != 0){
        var contact = assembleFromDTO(dto.NewContact, claimContact, claimContact.Bundle)
        var claimContactRoles = assembleFromDTOs(dto.NewRoles, claimContact, claim, contact.Bundle)
      }
