 ...
 */
@Export
@WsiExportable("http://guidewire.com/cc/ws/gw/webservice/cc/cc900/dto/ClaimDTO")
final class ClaimDTO {
  ...
  var _newLossLocation           : AddressDTO           as NewLossLocation
  ...
  /** Convenience property that returns a {@link Incident[]} of the objects whose PublicIDs are in {@code IncidentPublicIDs}. This property is only available on the server--it is not exposed through the WS-I layer. */
  property get Incidents()              : Incident[]          { return fetchByPublicID(IncidentPublicIDs) }
  
  /** Convenience property that returns the {@link Address} whose PublicID is {@code LossLocationPublicID}, or {@code null} if PublicID is {@code null}. This property is only available on the server--it is not exposed through the WS-I layer. */
  ...
  /** Convenience property that returns the {@link Contact} whose PublicID is {@code reporterPublicID}, or {@code null} if PublicID is {@code null}. This property is only available on the server--it is not exposed through the WS-I layer. */
  property get reporter()               : Contact             { return fetchByPublicID(reporterPublicID) }
  
  /** Convenience property that returns the {@link Contact} whose PublicID is {@code subrogatorPublicID}, or {@code null} if PublicID is {@code null}. This property is only available on the server--it is not exposed through the WS-I layer. */
