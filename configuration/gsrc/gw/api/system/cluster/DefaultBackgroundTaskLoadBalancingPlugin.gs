...
class DefaultBackgroundTaskLoadBalancingPlugin implements BackgroundTaskLoadBalancingPlugin, InitializablePlugin {
  ...
  private function readParameter<T>(parameters: Map, name: String, ref: PropertyReference<DefaultBackgroundTaskLoadBalancingPlugin, T>) {
  ...
  protected function rebalance(context: LoadBalancingContext): LoadBalancingResult {
    ...
    if (thisServer.getOwnedUpperBound() >= optimalCnt && isLessLoadedServerRebalancing(servers, thisServer)) {
      return ShortDelayAndRepeat
    }
